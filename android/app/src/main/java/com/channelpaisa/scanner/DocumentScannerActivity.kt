...
import com.channelpaisa.channelpaisa.R
import com.channelpaisa.scanner.constants.DefaultSetting
import com.channelpaisa.scanner.constants.DocumentScannerExtra
import com.channelpaisa.scanner.constants.ImageProvider
import com.channelpaisa.scanner.extensions.*
import com.channelpaisa.scanner.models.Document
import com.channelpaisa.scanner.models.Quad
import com.channelpaisa.scanner.ui.ImageCropView
...
class DocumentScannerActivity : AppCompatActivity() {
    ...
    private var letUserRotateImage =DefaultSetting.LET_USER_ROTATE_IMAGE
    /**
    ...
    private var rotatedDegree:Float = 0F
    /**
    ...
    private var cornersInImagePreview: Quad? = null
    /**
...
//            val photo: Bitmap = ImageUtil().getImageFromFilePath(originalPhotoPath)
            imageBitMap= ImageUtil().getImageFromFilePath(originalPhotoPath);
            // get document corners by detecting them, or falling back to photo corners with
            // slight margin if we can't find the corners
            val corners = try {
                val (topLeft, topRight, bottomLeft, bottomRight) = getDocumentCorners(imageBitMap!!)
            ...
            document = Document(originalPhotoPath, imageBitMap!!.width, imageBitMap!!.height, corners)
            ...
            if (letUserAdjustCrop) {
                ...
                try {
                    ...
                    imageView.setImagePreviewBounds(imageBitMap!!, screenWidth, screenHeight)
                    ...
                    val cornersInImagePreviewCoordinates = corners
                        .mapOriginalToPreviewImageCoordinates(
                            ...
                            imageView.imagePreviewBounds.height() / imageBitMap!!.height
                        )
                    cornersInImagePreview = corners
                            .mapOriginalToPreviewImageCoordinates(
                                    imageView.imagePreviewBounds,
                                    imageView.imagePreviewBounds.height() / imageBitMap!!.height
                            )
                    ...
                    imageView.setCropper(cornersInImagePreview!!)
                } catch (exception: Exception) {
                    finishIntentWithError(
...
//            }
    ...
    //Rotate the image
    private fun rotationImpl(){
        rotatedDegree+=90F
    }

    //Resets the rotation on complete
    private fun resetRotation(){
        println("Reset rotating")
        rotatedDegree=0F
    }
    ...
    //
    private fun saveCroppedImageToFile(bitmap: Bitmap) {
        // Create a file output stream for the temporary image file
        ...
        bitmap.compress(Bitmap.CompressFormat.JPEG, DefaultSetting.CROPPED_IMAGE_QUALITY, outputStream)
...
//        newPhotoButton.onClick { onClickNew() }
     ...
     */
    private var isOnDoneExecuting = false

    private fun onClickDone() {
        if (isOnDoneExecuting) {
            return
        }
        addSelectedCornersAndOriginalPhotoPathToDocuments()
         progressBar =  findViewById(R.id.progressBar);
        complete_document_scan_button.isEnabled=false;
        progressBar.visibility=View.VISIBLE;
        Log.d("CALLED_ORI","CaLLED_BEOFORE")
      if(croppingDocument==false)  cropDocumentAndFinishIntent()else return;
        complete_document_scan_button.isEnabled=true;
        Log.d("CALLED_ORI","CALLED_ADTER")
    ...
    private fun onClickRotate() {
        rotationImpl();
            val rotatedBitmap = imageBitMap?.let { rotateBitmap(it, rotatedDegree) }
            val (topLeft, topRight, bottomLeft, bottomRight) = rotatedBitmap!!.let { getDocumentCorners(it) }
            val corns =  Quad(topLeft, topRight, bottomRight, bottomLeft)


        // display cropper, and allow user to move corners
            document = Document(document!!.originalPhotoFilePath, rotatedBitmap.width, rotatedBitmap.height, corns)
        if (rotatedBitmap != null) {
            imageView.setImage(rotatedBitmap)
        }
            cornersInImagePreview = corns
                .mapOriginalToPreviewImageCoordinates(
                        imageView.imagePreviewBounds,
     ...
     * all document photos.
     */
    var croppingDocument = false;

    private fun cropDocumentAndFinishIntent() {
        croppingDocument=true;
        Intent().putExtra("croppingImage", true);
        ...
        for ((pageNumber, document) in documents.withIndex()) {
            ...
            val photoBitmap = ImageUtil().getImageFromFilePath(document.originalPhotoFilePath)
            val rotatedBitmap = rotateBitmap(photoBitmap, rotatedDegree)
            saveCroppedImageToFile(rotatedBitmap);
            val croppedImage: Bitmap = try {
                ...
                Intent().putExtra("croppingImage", false);
